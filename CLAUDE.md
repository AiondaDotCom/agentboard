...
- **Ticket in anderes Projekt verschieben**: `BoardService.moveTicketToProject()` – Ticket behaelt ID, Titel, Kommentare und Revisionen und landet am Ende der Zielspalte (Default: ERSTE Spalte des Zielprojekts, Spalten-IDs unterscheiden sich pro Projekt). Dependencies ueberqueren nie Projektgrenzen: eigene Deps werden geloescht (revisionssicher geloggt), haengen noch andere Tickets an dem Ticket, wird der Move mit `ConflictError` (409) abgelehnt. Gruppenname wandert mit; ist die Gruppe im Zielprojekt von einem anderen Agenten geclaimt, wird ein zugewiesenes Ticket abgelehnt. Events: `TICKET_DELETED` fuers Quellboard, `TICKET_CREATED` fuers Zielboard, Activity-Eintrag in BEIDEN Projekten. Zugaenge: MCP `move_ticket_to_project`, Batch-Op gleichen Namens, `PATCH /api/projects/:id/tickets/:ticketId/project` (Agent) und `POST .../project` (Human/UI). Frontend: Projekt-Dropdown oben im Details-Panel des Ticket-Modals (Auswahl aendern → Confirm → Move).
...
19 Tools: batch, list_projects, create_project, get_project, update_project, delete_project, list_tickets, get_ticket, create_ticket, update_ticket, move_ticket, move_ticket_to_project, assign_ticket, delete_ticket, add_comment, get_comments, get_ticket_history, list_agents, whoami
