...
- **Live Runtime Status**: `POST /api/runtime` nimmt mit einem dedizierten `X-Api-Key` Heartbeats von Host-Collectors an. Der Header zeigt wirklich laufende Codex-/Claude-Turns gruen, offene idle Prozesse nicht als arbeitend, plus die persistente Startzeit der Non-stop-Serie im Format `3 AIs working since 14:12`; nach 130 Sekunden ohne Heartbeat wird der Host als offline behandelt. Statuswechsel kommen per GraphQL-Subscription, ein 15s-Poll stellt TTL-/Reconnect-Updates sicher. Der macOS-Collector unter `scripts/` scannt alle 10s, sendet bei Aenderung bzw. alle 60s und laeuft per LaunchAgent beim Login.
